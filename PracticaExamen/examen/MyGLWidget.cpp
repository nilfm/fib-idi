...
//  glViewport (0, 0, ample, alt);
  ...
  modelTransformPatricioUpsidedown ();
  glDrawArrays(GL_TRIANGLES, 0, patr.faces().size()*3);
...
void MyGLWidget::modelTransformPatricioUpsidedown () {
  glm::mat4 TG(1.f);  // Matriu de transformació
...
{
  std::cout << fovIni << std::endl;
  std::cout << "raw: " << raw << std::endl;
  if (raw < 1.0) {
	fov = 2.0*atan(tan(fovIni/2.0)/raw);
  }
  else {
    fov = fovIni;  
  }
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  {
	fovIni = std::min(M_PI/3.0, std::max(M_PI/13, fovIni+(e->y() - yClick) * M_PI / 180.0));  
	projectTransform();  
	int slider = angleToSlider(fov);
	emit zoomChanged(slider);
  }
...
void MyGLWidget::changeZoom(int slider) {
	makeCurrent();
	fovIni = sliderToAngle(slider);
	std::cout << slider << " -> ";
	std::cout << fovIni << std::endl;
	projectTransform();
	update();
...
//n -> M_PI/(3+n/10)
float MyGLWidget::sliderToAngle(int slider) {
	return float(M_PI)/(3 + float(slider)/10);
}

//M_PI/(3+n/10) -> n
int MyGLWidget::angleToSlider(float angle) {
	float x = M_PI/angle;
	return 10*(x-3);
}
