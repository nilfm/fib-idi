...
//  glViewport (0, 0, ample, alt);
  ...
  glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);
  modelTransform(false, 2);
  glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);

  glBindVertexArray(VAO_Terra);
  modelTransform(true, 0);
...
void MyGLWidget::modelTransformPatricio() 
{
  // Matriu de transformació de model
	...
	if (not terra) {
		...
		else if (num_patricio == 2) {
			transform = glm::translate(transform, glm::vec3(-2, 0, -2));
			transform = glm::rotate(transform, float(M_PI), glm::vec3(0, 1, 0));
		}
		else {
			transform = glm::rotate(transform, float(M_PI)/2.0f, glm::vec3(0, 1, 0));
		}
...
void MyGLWidget::viewTransform() {
	...
	glm::mat4 View(1.0f);
	View = glm::translate(View, glm::vec3(0.0, 0.0, -2*radius));
...
void MyGLWidget::mouseReleaseEvent(QMouseEvent *e) {
	...
	if (e->x() - mousePos[0] > 25) {
		psi -= 0.1*((e->x()-mousePos[0])/50);
	}
	else if (mousePos[0] - e->x() > 25) {
		psi += 0.1*(-(e->x()-mousePos[0])/50);
	}
	if (mousePos[1] - e->y() > 25) {
		theta += 0.1*(-(e->y()-mousePos[1])/50);
	}
	else if (e->y() - mousePos[1] > 25) {
		theta -= 0.1*((e->y()-mousePos[1])/50);
	}
	update();
}
